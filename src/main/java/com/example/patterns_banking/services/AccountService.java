...
import com.example.patterns_banking.services.commands.ConsultAllAccountCommand;
...
public class AccountService {
    ...
    public Account deposit(Long accountId, Double amount) {
        ICommand<Account> command = new DepositCommand(accountRepository, accountId, amount);//creamos el comando
        //asignarlo a la interfaz me permite cambiar el comando (lado derecho) y podria seguir funcionando (agnostico)
        return command.execute();
    }

    public List<Account> findAllAccount() {
        ICommand<List<Account>> command = new ConsultAllAccountCommand(accountRepository);
        return command.execute();
    }
